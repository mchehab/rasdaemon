...
int handle_ras_events(int record_events)
{
	...
	if (rc == -255) {
		if (pthread_mutex_init(&ras->db_mutex, NULL) != 0) {
			log(SYSLOG, LOG_INFO, "sql db mutex init has failed\n");
			goto err;
		}
		...
		for (i = 0; i < cpus; i++) {
			...
			if (rc) {
				...
				pthread_mutex_destroy(&ras->db_mutex);
				goto err;
			}
		}
