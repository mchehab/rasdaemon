...
Only one compiled SQL backend is selected for a daemon process. Event modules
which persist data own a static ``db_desc_and_stmt`` pair, register it with
``ras_db_table_register`` during initialization, and unregister it during
cleanup. The descriptor and statement pair must remain valid while registered.
