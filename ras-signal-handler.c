...
static char *signal_res[] = {
	[TRACE_SIGNAL_DELIVERED] = "Deliverd",
...
{
	trace_seq_printf(s,
			 "signal: %s, errorno: %d, code: %s, comm: %s, pid: %d, grp: %d, res: %s, msg: %s",
			 strsignal(ev->sig), ev->error_no, errcode_str[ev->code], ev->comm, ev->pid,
			 ev->group, signal_res[ev->result], ev->sig == SIGBUS ? signal_msg[ev->code] : "Unknown");
}
